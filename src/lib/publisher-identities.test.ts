...
test('the fixed list has exactly the four authorized identities, Arjun Mehta first', () => {
  assert.deepEqual(
    PUBLISHER_IDENTITIES.map((i) => i.key),
    ['ARJUN_MEHTA', 'KARAN_SHAH', 'NEEL_KAPOOR', 'VIKRAM_RAO'],
  )
  assert.equal(PUBLISHER_IDENTITIES[0].label, 'Arjun Mehta')
})
...
test('a missing or unrecognized identity falls back to the first identity, never blank or raw', () => {
  assert.equal(publisherIdentityLabel(undefined), 'Arjun Mehta')
  ...
  assert.equal(publisherIdentityLabel('SOMETHING_UNKNOWN'), 'Arjun Mehta')
})
