...
import {
  previewAdminGrantImport,
  startAdminGrantImport,
  getAdminGrantImport,
  listAdminGrantImports,
  listAdminGrantImportIssues,
  type AdminGrantImportPreview,
  type AdminGrantImportBatch,
...
function BatchIssuesModal({ batch, onClose }: { batch: AdminGrantImportBatch; onClose: () => void }) {
  const [page, setPage] = useState(0)
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['admin', 'grants', 'import', batch.id, 'issues', page],
    queryFn: () => listAdminGrantImportIssues(batch.id, { page, size: 20 }),
  })
                ...
                <div key={issue.id} className="flex items-start gap-2.5 rounded-lg border border-border/70 p-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-fg-muted">
                      Row {issue.rowNumber}
                      {issue.grantName && ` · ${issue.grantName}`}
                    </p>
...
function BatchReportCard({ batch, onViewIssues }: { batch: AdminGrantImportBatch; onViewIssues: () => void }) {
    ...
    <Card className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="font-medium text-fg truncate">{batch.originalFilename ?? 'Grant import'}</p>
...
function ImportHistory({ onViewIssues }: { onViewIssues: (batch: AdminGrantImportBatch) => void }) {
  const [page, setPage] = useState(0)
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['admin', 'grants', 'import', 'history', page],
    queryFn: () => listAdminGrantImports({ page, size: 10 }),
  })
            ...
            {data.content.map((batch) => (
              <Card key={batch.id} padding="sm" className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2.5">
                  <FileSpreadsheet className="size-4 text-fg-muted shrink-0" />
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-fg truncate">{batch.originalFilename ?? 'Grant import'}</p>
                    <p className="text-xs text-fg-muted">
                      {batch.createdCount} created · {batch.skippedCount} skipped · {batch.failedCount} failed ·{' '}
...
/** The manual, zero-API-cost way to populate the Grants list in bulk — upload a CSV/Excel file
 *  instead of the (currently disabled) Gemini discovery pipeline paying per call. Two deliberate
 *  steps: preview (parses and validates, saves nothing) then confirm (actually imports, off-request
 *  — see GrantImportService on the backend). Every imported row is live immediately, same as any
 *  other admin-created grant; there's no dedup against existing grants, so re-uploading the same
 *  file creates duplicates — the preview step is where that gets caught before confirming. */
export function AdminGrantImportPanel() {
  const queryClient = useQueryClient()
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [pickedFile, setPickedFile] = useState<File | null>(null)
  const [preview, setPreview] = useState<AdminGrantImportPreview | null>(null)
  const [activeBatchId, setActiveBatchId] = useState<string | null>(null)
  const [issuesBatch, setIssuesBatch] = useState<AdminGrantImportBatch | null>(null)

  const previewMutation = useMutation({
    mutationFn: (file: File) => previewAdminGrantImport(file),
  ...
  const startMutation = useMutation({
    mutationFn: (file: File) => startAdminGrantImport(file),
    onSuccess: (batch) => {
      setActiveBatchId(batch.id)
      setPreview(null)
      setPickedFile(null)
      if (fileInputRef.current) fileInputRef.current.value = ''
      queryClient.invalidateQueries({ queryKey: ['admin', 'grants', 'import', 'history'] })
  ...
  const batchQuery = useQuery({
    queryKey: ['admin', 'grants', 'import', activeBatchId],
    queryFn: () => getAdminGrantImport(activeBatchId!),
      ...
      <Card className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-fg-muted">
            Research schemes yourself (any AI chat tool, or manual search) and paste them into a spreadsheet
            with columns name, provider, provider_type, description, funding_amount, eligibility_criteria,
            eligible_sectors, eligible_stages, application_deadline, application_url — then upload it here.
            Every row becomes a live grant immediately. Re-uploading the same file always creates new rows,
            so double-check the preview below before confirming.
          </p>
        </div>
        ...
        {!pickedFile && !batchQuery.data && (
          <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border/80 bg-surface-sunken/40 px-6 py-10 text-center transition-colors hover:border-brand-400 hover:bg-brand-500/5">
            ...
            <span className="text-xs text-fg-muted">name, provider, provider_type, deadline, application_url, and the other grant columns</span>
            <input
              ref={fileInputRef}
              type="file"
        ...
        {preview && pickedFile && (
          <div className="flex flex-col gap-4">
                ...
                <tbody className="divide-y divide-border/60">
                  {preview.sampleRows.map((row) => (
                    <tr key={row.rowNumber} className={row.error ? 'bg-danger-500/10' : undefined}>
                      ...
                      <td className="px-3 py-2 text-fg-secondary">{[row.provider, row.providerType].filter(Boolean).join(' · ') || '—'}</td>
                      <td className="px-3 py-2 text-fg-secondary">{row.deadline ?? 'Rolling'}</td>
            ...
            <Button isLoading={startMutation.isPending} onClick={() => startMutation.mutate(pickedFile)} className="self-start">
              Import {preview.totalRows} grant{preview.totalRows === 1 ? '' : 's'}
            </Button>
          </div>
        )}
