...
import { uploadEventCoverImage } from '@/services/events.service'
import { toast } from '@/store/toast.store'

const ACCEPT = 'image/png,image/jpeg,image/webp,image/gif'
/** Matches the server's limit for event covers. */
...
interface EventCoverFieldProps {
  ...
  /** Lets the form hold its Save button until an upload has finished, so a cover is never silently dropped. */
  onUploadingChange?: (uploading: boolean) => void
}

/**
 * The cover picture of an event: upload a file, or paste a link to one. Either way the form ends up with a
 * URL, so a cover can be chosen while the event is still being written.
 */
export function EventCoverField({ value, onChange, onUploadingChange }: EventCoverFieldProps) {
  ...
  async function handleFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      onChange(await uploadEventCoverImage(file))
      setPhase('done')
