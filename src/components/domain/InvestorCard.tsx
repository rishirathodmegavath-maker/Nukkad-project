...
export function InvestorCard({ investor, onRequestIntro }: { investor: InvestorProfile; onRequestIntro?: () => void }) {
  ...
  return (
    <Card interactive padding="sm" className="flex flex-col gap-3 border border-border/80 shadow-xs hover:border-border-strong transition-all min-w-0 overflow-hidden bg-surface">
      {/* thesis/badges/ticket used to sit outside this Link — interactive's cursor-pointer/hover
          made that area look clickable, but it wasn't. The "Request introduction" button below is a
          real separate action, so it deliberately stays outside the link instead of being absorbed. */}
      <Link to={`/investors/${investor.id}`} className="flex flex-col gap-3">
